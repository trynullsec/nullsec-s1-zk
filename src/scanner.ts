...
export async function scanTarget(target: string, options: ScanOptions = {}): Promise<ScanRunResult> {
...
function defaultReportPath(format: OutputFormat): string {
  if (format === "markdown") return "nullsec-zk-report.md";
  if (format === "sarif") return "nullsec-zk-report.sarif";
  if (format === "json") return "nullsec-zk-report.json";
