import type { Assignment, Rule } from "../types.js";
import { buildIssue } from "../core/issue-builder.js";
import { expressionHasDivisionOrInverse } from "../frontends/circom/circom-utils.js";

function denominatorOf(expression: string): string | undefined {
  const match = expression.match(/\/\s*([A-Za-z_][A-Za-z0-9_.\[\]]*)/);
  return match?.[1];
}

function hasNonzeroGuard(assignment: Assignment, denominator: string | undefined, context: Parameters<Rule["analyze"]>[0]): boolean {
  if (!denominator) return /Safe|NonZero|AssertNonZero/i.test(assignment.rhs);
...
export const unsafeDivisionOrInverseRule: Rule = {
  id: "NS-ZK-008",
