...
export const missingRangeCheckRule: Rule = {
  id: "NS-ZK-006",
  ...
  analyze(context) {
    ...
    for (const signal of context.graph.declaredSignals()) {
      ...
      if (!assessment.shouldReport) continue;
      ...
      issues.push(
        buildIssue({
          ...
          impact:
            "Field elements are not naturally bounded integers. Missing range checks can cause overflows, aliasing, invalid comparisons, or inconsistent protocol semantics.",
          suggestedFix:
            "Constrain the value with an explicit bit decomposition, range-check gadget, comparator, or protocol-specific bounded representation.",
