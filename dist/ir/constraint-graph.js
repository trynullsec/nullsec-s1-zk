...
export class ConstraintGraph {
    ...
    hasRangeCheck(signalName) {
        ...
        const rangeComponentNames = new Set(this.ir.components.filter((component) => rangePattern.test(`${component.templateType ?? ""}${component.name}`)).map((component) => component.name));
        return (this.ir.components.some((component) => rangePattern.test(`${component.templateType ?? ""}${component.name}`) && component.inputs.some((input) => expressionMentionsSignal(input, signalName))) ||
            this.ir.constraints.some((constraint) => constraint.lhs !== undefined &&
